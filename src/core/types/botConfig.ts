...
export interface BotConfig {
	chainPrefix: string;
	rpcUrl: string;
	poolEnvs: Array<{ pool: string; fee: number }>;
	...
	skipBidPercentage?: number | undefined;
}

/**
 * Set the bot configuration settings from environment variables.
 * @param ensv The environment variables.
 * @returns BotConfig.
 */
export function setBotConfig(envs: NodeJS.ProcessEnv): BotConfig {
	...
	const GAS_PRICE = GasPrice.fromString("" + GAS_UNIT_PRICE + "" + envs.BASE_DENOM);
	const SKIP_BID_PERCENTAGE = envs.SKIP_BID_PERCENTAGE !== undefined ? +envs.SKIP_BID_PERCENTAGE : undefined;
	const botOptions: BotConfig = {
		...
		skipBidPercentage: SKIP_BID_PERCENTAGE,
	};
	return botOptions;
}

/**
 * Validate environment for existence of required variables.
